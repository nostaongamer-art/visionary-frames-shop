...
import { CustomProductSection } from "./BestSellers";
import { 
  Truck, 
  ShieldCheck, 
  CreditCard, 
...
export function CategoryPageLayout({ pageId: propPageId }: CategoryPageLayoutProps) {
  ...
  const heightClass = (() => {
    switch (bannerHeightSetting) {
      case "small": return "h-[180px]";
      case "medium": return "h-[300px]";
      case "large": return "h-[400px]";
      case "giant":
      default:
        return "h-[220px] sm:h-[350px] md:h-[600px]";
    }
  })();
  ...
  const titleSizeClass = (() => {
    switch (bannerHeightSetting) {
      ...
      case "medium": return "text-2xl sm:text-4xl";
      case "large": return "text-2xl sm:text-4xl md:text-5xl";
      case "giant":
      default:
  ...
  const subtitleSizeClass = (() => {
    switch (bannerHeightSetting) {
      ...
      case "medium": return "text-xs sm:text-sm";
  ...
  const showProducts = pageData.productsShow !== false;
  ...
  const renderHeader = () => showBanner && (
    <section className={`relative ${heightClass} bg-[#090B0E] overflow-hidden`}>
      <div className="mx-auto max-w-[1240px] px-4 sm:px-6 h-full flex items-center relative z-10">
        <div className="max-w-md md:max-w-2xl text-left py-6 flex flex-col justify-center h-full">
          ...
          <h1 className={`${titleSizeClass} font-extrabold tracking-tight text-white mb-2 md:mb-3.5 font-display`}>
            {(() => {
          ...
          <p className={`${subtitleSizeClass} text-white/70 max-w-sm md:max-w-xl`}>
            {headerData.subtitle}
          </p>
        </div>
      </div>
  ...
  const renderBenefits = () => showBenefits && benefitsList.length > 0 && (
  ...
  const renderProducts = () => showProducts && (
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-2.5 right-2.5 h-6 w-6 rounded-full bg-background/85 hover:bg-background border border-border/60 shadow-sm flex items-center justify-center text-muted-foreground hover:text-red-500 z-10 transition-colors cursor-pointer"
                    >
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-brand selection:text-white">
      ...
      {pageData.sectionOrder?.map((secKey) => {
        if (secKey === "header") return <div key="header">{renderHeader()}</div>;
        if (secKey === "benefits") return <div key="benefits">{renderBenefits()}</div>;
        if (secKey === "products") return <div key="products">{renderProducts()}</div>;
        if (secKey.startsWith("custom-sec-")) {
          const customId = secKey.replace("custom-sec-", "");
          const customSec = pageData.customSections?.find((s) => s.id === customId);
          if (customSec) {
