...
export function PersonalDataForm({ values, errors, onChange }: PersonalDataFormProps) {
  ...
  const handleCpfChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (input.length > 9) {
      formatted = `${input.substring(0, 3)}.${input.substring(3, 6)}.${input.substring(6, 9)}-${input.substring(9)}`;
    }
  ...
  return (
    <div className="w-full flex flex-col gap-4 select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* CPF */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="cpf" className="text-xs font-semibold text-ink">
            CPF
          </label>
          <input
            id="cpf"
            ...
            className={`w-full h-[38px] px-3 bg-white border ${
              errors.cpf ? "border-red-500" : "border-[#D9DDE2]"
            } rounded-[4px] text-sm text-ink outline-none focus:border-brand transition-colors`}
            placeholder="000.000.000-00"
          />
          {errors.cpf && (
            <span className="text-[10px] text-red-500 font-semibold">{errors.cpf}</span>
          )}
        </div>
      </div>
